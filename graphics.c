...
#endif
            printf("%cCliquer dans la fen�tre ... %4d %4d           %c", 13, event.motion.x, HEIGHT - event.motion.y,
                   13);
            fflush(stdout);
            encore = 0;
            P.x = event.button.x;
...
void init_reseau() {
    ...
    if (SDLNet_Init() == -1) {
        printf(" *** Erreur d'iniatialisation : %s\n", SDLNet_GetError());
        exit(1);
    }
...
// 9.2 Ouverture/Fermeture d'un serveur au port indique
...
int connexion_client_serveur(char *adresse_ip, unsigned int port) {
    SDLNet_ResolveHost(&ip_client, adresse_ip, port); //Remplit les champs de la structure IPaddress convenablement
    if ((client = SDLNet_TCP_Open(&ip_client)))
        return 1;
    else
        return 0;
}
...
// char* message_recu;
char *lire_message() {
    char *reponse = malloc(_TAILLE_RESEAU_TRANSFERT_ * sizeof(char));
...
void envoyer_message(char *message) {
    if (strlen(message) + 1 > _TAILLE_RESEAU_TRANSFERT_) {
        printf(" *** Taille du texte trop grande!\n");
        return;
    }
...
int lire_nombre() {
    int reponse;
    SDLNet_TCP_Recv(client, &reponse, sizeof(int));
    return reponse;
}
...
const char *ip_du_serveur() {
    //const char* addrip;
    if (serveur != NULL) {
        IPaddress *ip = SDLNet_TCP_GetPeerAddress(serveur);
        if (ip != NULL) {
...
unsigned int port_du_serveur() {
    if (serveur != NULL) {
        IPaddress *ip = SDLNet_TCP_GetPeerAddress(serveur);
        if (ip != NULL)
            return (unsigned int) SDLNet_Read16(&(ip->port));
    }
    return 0;
}
